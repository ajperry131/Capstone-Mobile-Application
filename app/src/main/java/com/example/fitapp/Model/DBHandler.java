...
public class DBHandler extends SQLiteOpenHelper {
    ...
    public void deleteWorkoutById(int id) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete("workout", "id=?", new String[] {String.valueOf(id)});
    }

    @SuppressLint("Range")
    public ArrayList<Workout> getWorkoutsFromUserId(int userId) {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.rawQuery("SELECT * FROM workout WHERE user_id=?", new String[] {String.valueOf(userId)});
        ArrayList<Workout> workouts = new ArrayList<>();
        try {
            while (cursor.moveToNext()) {
                Workout workout = new Workout();
                workout.setId(cursor.getInt(cursor.getColumnIndex("id")));
                workout.setUserId(cursor.getInt(cursor.getColumnIndex("user_id")));
                workout.setWorkoutName(cursor.getString(cursor.getColumnIndex("workout_name")));
    ...
    @SuppressLint("Range")
    public String getTypeOfExercise(String exerciseName) {

        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM exercise WHERE name=?", new String[] {exerciseName});

        try {
            if (cursor != null && cursor.moveToFirst()) {
                return cursor.getString(cursor.getColumnIndex("exercise_type"));
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    @SuppressLint("Range")
    public String getGifUrlOfExercise(String exerciseName) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM exercise WHERE name=?", new String[] {exerciseName});

        try {
            if (cursor != null && cursor.moveToFirst()) {
                return cursor.getString(cursor.getColumnIndex("exercise_gif_url"));
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    public boolean updateWorkoutExerciseById(int id, int sets, int reps, float weight) {
        SQLiteDatabase db = this.getReadableDatabase();
        try {
            ContentValues values = new ContentValues();

            values.put("workout_exercise_sets", sets);
            values.put("workout_exercise_reps", reps);
            values.put("workout_exercise_weight", weight);

            db.update("workout_exercise", values, "id=?", new String[]{String.valueOf(id)});
            db.close();
            return true;
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public void deleteWorkoutExerciseById(int id) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete("workout_exercise", "id=?", new String[] {String.valueOf(id)});
    }
    @SuppressLint("Range")
    public ArrayList<WorkoutExercise> getExercisesOfWorkout(int workoutId) {
        SQLiteDatabase db = this.getReadableDatabase();;
        Cursor cursor = db.rawQuery("SELECT * FROM workout_exercise WHERE workout_id=?", new String[] {String.valueOf(workoutId)});
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                exercise.setWorkoutId(cursor.getInt(cursor.getColumnIndex("workout_id")));
                exercise.setExerciseName(cursor.getString(cursor.getColumnIndex("exercise_name")));
                exercise.setWorkoutExerciseSets(cursor.getInt(cursor.getColumnIndex("workout_exercise_sets")));
                exercise.setWorkoutExerciseReps(cursor.getInt(cursor.getColumnIndex("workout_exercise_reps")));
                exercise.setWorkoutExerciseWeight(cursor.getFloat(cursor.getColumnIndex("workout_exercise_weight")));
                exercises.add(exercise);
            }
            return exercises;
        } catch (Exception e) {
    ...
    public void updateMealById(int id, int userId, String mealName, int mealCalories, int mealProtein, Calendar mealDate) {
        ...
        db.update("meal", values, "id=?", new String[] {String.valueOf(id)});
    }

    public void deleteMealById(int id) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete("meal", "id=?", new String[] {String.valueOf(id)});
    }
    ...
    public ArrayList<Meal> getMealsOfUserByDate(int userId, Calendar mealDate) {
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                meal.setDate(cursor.getString(cursor.getColumnIndex("meal_date")));
                meals.add(meal);
            }
            return meals;
        } catch (Exception e) {
